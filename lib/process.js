...
function lazyProcessContainer(htmlContent) {
    ...
    images.forEach(function (image) {
        ...
        htmlContent = htmlContent.replace(regex, function (tag, pre, url, post) {
            ...
            // new html code
            var result = '<div class="img-container" style="width:' + image.width + 'px;background:url(' + loadingImage + ');background-repeat:no-repeat;background-position:center;">' +
...
module.exports.processPost = function (data) {
    if(this.config.lazyload.post.excludeEncrypt && this.config.encrypt && this.config.encrypt.enable && data.password){
        return data;
    }
